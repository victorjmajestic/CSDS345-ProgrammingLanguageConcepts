...
(define interpret-statement-list
  (lambda (statement-list environment return break continue throw)
    (if (null? statement-list)
        ...
        (eval-mainFunction environment return break continue throw)
        (interpret-statement-list (rightSubtree statement-list) (interpret-statement (leftSubtree statement-list) environment return break continue throw) return break continue throw))))
...
(define eval-mainFunction
  (lambda (environment return break continue throw)
    (if (exists? 'main environment)
      (interpret-statement-list (funcStatements (lookup 'main environment)) (push-frame environment) return break continue throw) 
      (myerror "Main does not exist"))))
...
(define interpret-funcall_M_State
 (lambda (statements environment return break continue throw)
  (cond
    ...
    (else
     (cond
       ((not (list? (call/cc (lambda (break_return) (interpret-statement (leftSubtree statements) environment break_return break continue throw)))))
             environment)
       (else (interpret-funcall_M_State (rightSubtree statements) (call/cc (lambda (break_return)
                                                                         (interpret-statement (leftSubtree statements) environment break_return break continue throw)))
                                         return break continue throw)))))))
  ...
  (lambda (funcall environment throw)
    (call/cc
     (lambda (functionReturn)
       (cond
         ((exists? (functionName funcall) environment)
          (if (not (null? (cdr funcall)))      
               ...
               (pop-frame (interpret-block-list (funcStatements (lookup (functionName funcall) environment))
                                                  ...
                                                  (lambda (env) (myerror "Continue used outside of loop"))
                                                  throw))))
         (else (myerror "Function does not exist")))))))
...
(define interpret-block-list
  (lambda (statement-list environment return break continue throw)
    (if (null? statement-list)
        environment
        (interpret-block-list (rightSubtree statement-list) (interpret-statement (leftSubtree statement-list) environment return break continue throw) return break continue throw))))
